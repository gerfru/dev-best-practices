...
## 8. Formal Methods & AI System Security

### 8.1 Missing Input Validation for ML Models (Distribution Shift)

**CWE-20 + EU AI Act Art. 15**

Check for:

- ML model deployed without runtime monitoring for out-of-distribution inputs
- No confidence threshold below which the model defers to a human or returns an error
- No check that input feature ranges match training distribution at inference time
- Model outputs used in safety-critical decisions without uncertainty quantification

**Why it matters:** A neural network classifier trained on data from one distribution will produce confident but wrong predictions on inputs outside that distribution — this is called distribution shift. A medical diagnosis model trained on patients from one hospital may silently fail on patients from another. The EU AI Act (Art. 15) requires high-risk AI systems to have appropriate monitoring. Use runtime monitors that detect when inputs fall outside the training distribution and flag them for human review.

**Reference:** [ISEC Trustworthy AI](https://www.isec.tugraz.at/course/trustworthy-artificial-intelligence-705007-sommersemester-2026/) | [CMU 15783 Trustworthy AI](https://csd-web-01.andrew.cmu.edu/course/15783/f25)

---

### 8.2 Non-Constant-Time Crypto Implementation

**CWE-208 (extended)**

Check for (beyond basic timing checks in section 1.5):

- Crypto code with secret-dependent table lookups: `S_BOX[key_byte]` — cache timing attack
- Secret-dependent loop bounds: `for i in range(secret_value):`
- Secret-dependent early returns in crypto operations
- Use of division or modulo with secret operands (variable-time on most CPUs)
- Python/interpreted language for performance-critical crypto paths (extremely hard to make constant-time)

**Why it matters:** Even if the algorithm is correct, the implementation leaks secrets through timing. Table lookups like `S_BOX[key[i]]` access different cache lines depending on the key byte — a network attacker measuring response times can recover AES keys from millions of queries. The correct approach is bitsliced implementations (process multiple inputs simultaneously using bitwise ops, no table lookups) or use a verified framework like Jasmin that mathematically proves constant-time.

**Reference:** [ISEC Cryptography on Software Platforms](https://www.isec.tugraz.at/course/cryptography-on-software-platforms-705219-sommersemester-2026/) | [MIT 6.566 Lec 5](https://css.csail.mit.edu/6.858/2024/)

---

### 8.3 Container / Cloud Isolation Failures

**CWE-269 + CWE-284**

Check for:

- Docker containers running as root with no `--user` flag
- `--privileged` flag used (disables all isolation)
- Host volumes mounted with write access unnecessarily: `-v /etc:/etc`
- seccomp profile disabled: `--security-opt seccomp=unconfined`
- Capabilities not dropped: container retains `CAP_SYS_ADMIN`, `CAP_NET_ADMIN`
- Container image pulling from `latest` tag (no pinned digest)
- Secrets passed as environment variables (visible in `docker inspect`, process listings)

**Why it matters:** A `--privileged` container has complete access to the host system — it's equivalent to running as root on the host. CVE-2019-5736 allowed a malicious container to overwrite the runc binary on the host, escaping the container entirely. Environment variables are readable by all processes in the container, logged in crash reports, and visible in orchestration dashboards. Use Docker secrets or Kubernetes secrets with projected volumes.

**Reference:** [ISEC Cloud Operating Systems](https://www.isec.tugraz.at/course/cloud-operating-systems-705050-sommersemester-2026/) | [MIT 6.566 Lec 2-3](https://css.csail.mit.edu/6.858/2024/)
